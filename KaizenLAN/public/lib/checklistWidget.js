...
async function displayChecklistWidget(element, checklistName, checklistItems) {
    const checklistContentDiv = document.createElement('div');
    checklistContentDiv.id = `${checklistName}Div`;
    checklistContentDiv.className = 'center';

    // Create header
    const header = document.createElement('h2');
    header.textContent = checklistName.replace(/_/g, ' ');
    checklistContentDiv.appendChild(header);
    ...
    for(let item of checklistItems) {
        ...
        checkbox.addEventListener('click', async function(event) {
            if(event.shiftKey) {
                ...
                label.classList.remove('checkBoxIncomplete');
                await ChecklistLoader.completeChecklistItem(item.id, checklistName);
                window.location.reload(true);
            }
        });
    ...
    button.addEventListener('click', async () => {
        var textFieldValue = itemTextField.value.trim();
        if(textFieldValue) {
            await ChecklistLoader.createChecklistItem(textFieldValue, checklistName);
            window.location.reload(true);
        }
    });
    element.appendChild(checklistContentDiv);
}

export { displayChecklist,  displayChecklistWidget }